...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # Forward Pass
        h0, h_cache = fc_forward(features, W_proj, b_proj)
        w_out, w_cache = word_embedding_forward(captions_in, W_embed)
        w_out2 = w_out.transpose(1, 0, 2)
        rnn_h, rnn_cache = rnn_forward(w_out2, h0, Wx, Wh, b)
        rnn_h = rnn_h.transpose(1, 0, 2)
        f_out, f_cache = temporal_fc_forward(rnn_h, W_vocab, b_vocab)
        loss, s_dx = temporal_softmax_loss(f_out, captions_out, mask)
        dx, dw, db = temporal_fc_backward(s_dx, f_cache)
        grads['W_vocab'] = dw
        grads['b_vocab'] = db
        ...
        _, dW, db = fc_backward(dh0, h_cache)
        grads['W_proj'] = dW
        grads['b_proj'] = db
