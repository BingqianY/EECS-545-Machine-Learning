...
def temporal_fc_forward(x, w, b):
    ...
    N, T, D = x.shape
    _,M = w.shape
    out = np.dot(x.reshape(N*T, D), w) + b
...
def temporal_softmax_loss(x, y, mask):
    ...
    loss, dx = None, None  
    N, T, V = x.shape
    x_max = np.max(x.reshape(N*T, V), axis = 1, keepdims = True)
    p = np.exp(x.reshape(N*T, V) - x_max)
    p = p/np.sum(p, axis = 1, keepdims = True)
    loss = 0
    #dx = np.zeros(p.shape[0])
    loss = -np.sum(mask.reshape(N*T)*np.log(p[range(N*T), y.reshape(N*T)])) / N
    #m = y.shape[0]
    grad = p.copy()
    grad[range(N*T),y.reshape(N*T)] -= 1
    grad = grad/N
    grad = grad*mask.reshape(N*T, 1)
    dx = grad.reshape(N, T, V)
